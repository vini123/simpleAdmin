      ...
      <el-table :data="tableData" ref="dragTable" class="w-full">
        ...
        <el-table-column label="操作" align="center" fixed="right" width="200">
          <template #default="scope">
            ...
            <el-button v-if="includes(app.routeNames, ['permission.edit'])" link size="small" type="primary" text @click="goEdit(scope.row)"
              >编辑</el-button
            >
            <el-button v-if="includes(app.routeNames, ['permission.delete'])" link size="small" type="primary" text @click="goDelete(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
function goDelete(value: any): void {
  if (value && value.id) {
    ElMessageBox.confirm("删除权限，子权限也会一起删除。确认删除吗？", "", {
